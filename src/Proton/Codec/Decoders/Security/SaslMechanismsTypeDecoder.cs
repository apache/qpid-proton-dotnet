...
{
   public sealed class SaslMechanismsTypeDecoder : AbstractDescribedListTypeDecoder
   {
      ...
      protected override SaslMechanisms ReadType(int count, IProtonBuffer buffer, IDecoder decoder, IDecoderState state)
      {
      ...
      protected override SaslMechanisms ReadType(int count, Stream stream, IStreamDecoder decoder, IStreamDecoderState state)
      {
         SaslMechanisms result = new();
