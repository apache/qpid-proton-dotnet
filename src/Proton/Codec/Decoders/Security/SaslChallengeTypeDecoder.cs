...
{
   public sealed class SaslChallengeTypeDecoder : AbstractDescribedTypeDecoder
   {
      ...
      public override Type DecodesType => typeof(SaslChallenge);
      ...
      private static SaslChallenge ReadType(IProtonBuffer buffer, IDecoderState state, IListTypeDecoder listDecoder)
      {
         SaslChallenge result = new SaslChallenge();
         ...
         {
            throw new DecodeException("SASL Challenge must contain at least one entry: " + count);
         }
         else
         {
            result.Challenge = state.Decoder.ReadBinary(buffer, state);
         }
      ...
      private static SaslChallenge ReadType(Stream stream, IStreamDecoderState state, IListTypeDecoder listDecoder)
      {
         SaslChallenge result = new SaslChallenge();
         ...
         {
            result.Challenge = state.Decoder.ReadBinary(stream, state);
         }
