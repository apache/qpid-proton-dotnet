...
{
   public sealed class SymbolElement : AtomicElement
   {
      private readonly Symbol value;

      public SymbolElement(IElement parent, IElement prev, Symbol value) : base(parent, prev)
      {
         this.value = value;
      }
      ...
      public override uint Encode(BinaryWriter writer)
      {
         ...
         {
            writer.Write(((byte)EncodingCodes.Sym8));
            writer.Write((byte)value.Length);
         }
         else
         {
            writer.Write(((byte)EncodingCodes.Sym32));
            writer.Write((byte)value.Length);
         }
