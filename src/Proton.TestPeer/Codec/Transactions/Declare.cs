...
namespace Apache.Qpid.Proton.Test.Driver.Codec.Transactions
{
   ...
   public sealed class Declare : ListDescribedType
   {
      ...
      public Declare(object described) : base(Enum.GetNames(typeof(DeclareField)).Length, (IList)described)
      {
      }

      public Declare(IList described) : base(Enum.GetNames(typeof(DeclareField)).Length, described)
      {
      }
      ...
      {
         get => (byte[])List[((int)DeclareField.GlobalId)];
         set => List[((int)DeclareField.GlobalId)] = value;
      }
