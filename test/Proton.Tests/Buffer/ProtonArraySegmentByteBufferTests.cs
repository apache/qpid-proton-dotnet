...
{
   [TestFixture]
   public class ProtonArraySegmentByteBufferTests : ProtonAbstractByteBufferTests
   {
      [SetUp]
      ...
      [Test]
      public new void TestConstructWithDifferingCapacityAndMaxCapacity()
      {
         base.TestConstructWithDifferingCapacityAndMaxCapacity();
      }
      ...
      public void TestConstructorByteArray()
      {
         ...
         IProtonBuffer buffer = new ProtonByteBuffer(source, source.Length);

         Assert.AreEqual(0, buffer.ReadableBytes);

         buffer.WriteOffset = source.Length;
      ...
      [Test]
      public void TestCannotIncreaseCapacityWhenAssignedArrayAndMaxCapacityOfArrayLength()
      {
      ...
      private static readonly int ARRAY_OVERRUN = 63;

      protected override IProtonBuffer AllocateBuffer(int initialCapacity)
      {
         byte[] initialArray = new byte[initialCapacity + ARRAY_OFFSET + ARRAY_OVERRUN];
         return new ProtonByteBuffer(initialArray, ARRAY_OFFSET, initialCapacity, Int32.MaxValue);
      }

      protected override IProtonBuffer AllocateBuffer(int initialCapacity, int maxCapacity)
      {
         byte[] initialArray = new byte[initialCapacity + ARRAY_OFFSET + ARRAY_OVERRUN];
         return new ProtonByteBuffer(initialArray, ARRAY_OFFSET, initialCapacity, maxCapacity);
      }
