...
{
   [TestFixture]
   public class CoordinatorTypeCodecTest : CodecTestSupport
   {
      [Test]
      public void TestTypeClassReturnsCorrectType()
      {
         Assert.AreEqual(typeof(Coordinator), new CoordinatorTypeDecoder().DecodesType);
         Assert.AreEqual(typeof(Coordinator), new CoordinatorTypeEncoder().EncodesType);
      }
      ...
      public void TestDescriptors()
      {
         ...
         Assert.AreEqual(Coordinator.DescriptorCode, decoder.DescriptorCode);
         Assert.AreEqual(Coordinator.DescriptorCode, encoder.DescriptorCode);
         Assert.AreEqual(Coordinator.DescriptorSymbol, decoder.DescriptorSymbol);
         Assert.AreEqual(Coordinator.DescriptorSymbol, encoder.DescriptorSymbol);
      }

      [Test]
      public void TestEncodeDecodeCoordinatorType()
      {
         TestEncodeDecodeCoordinatorType(false);
      }

      [Test]
      public void TestEncodeDecodeCoordinatorTypeFromStream()
      {
         TestEncodeDecodeCoordinatorType(true);
      }

      private void TestEncodeDecodeCoordinatorType(bool fromStream)
      {
         IProtonBuffer buffer = ProtonByteBufferAllocator.Instance.Allocate();
         Stream stream = new ProtonBufferInputStream(buffer);

         Symbol[] capabilities = new Symbol[] { Symbol.Lookup("Cap-1"), Symbol.Lookup("Cap-2") };
      ...
      [Test]
      public void TestEncodeDecodeCoordinatorTypeNoCapabilities()
      {
         TestEncodeDecodeCoordinatorTypeWithNoCapabilities(false);
      }

      [Test]
      public void TestEncodeDecodeCoordinatorTypeNoCapabilitiesFromStream()
      {
         TestEncodeDecodeCoordinatorTypeWithNoCapabilities(true);
      }

      private void TestEncodeDecodeCoordinatorTypeWithNoCapabilities(bool fromStream)
      {
         IProtonBuffer buffer = ProtonByteBufferAllocator.Instance.Allocate();
         Stream stream = new ProtonBufferInputStream(buffer);
         Coordinator input = new Coordinator();
      ...
      private void TestSkipValue(bool fromStream)
      {
         ...
         coordinator.Capabilities = new Symbol[] { Symbol.Lookup("skip") };
         ...
         coordinator.Capabilities = new Symbol[] { Symbol.Lookup("read") };
         ...
         for (int i = 0; i < 10; ++i)
         {
            if (fromStream)
            {
               ...
               Assert.AreEqual(typeof(Coordinator), typeDecoder.DecodesType);
         ...
         Assert.IsTrue(result is Coordinator);

         Coordinator value = result;
         Assert.AreEqual(1, value.Capabilities.Length);
         Assert.AreEqual(Symbol.Lookup("read"), value.Capabilities[0]);
      }
      ...
      private void DoTestSkipValueWithInvalidMapType(EncodingCodes mapType, bool fromStream)
      {
         ...
         buffer.WriteUnsignedByte(((byte)Coordinator.DescriptorCode));
      ...
      private void DoTestDecodeWithInvalidMapType(EncodingCodes mapType, bool fromStream)
      {
         ...
         try
         {
            ...
            Assert.Fail("Should have Assert.Failed to decode the Coordinator");
         }
         catch (DecodeException)
         {
         }
      ...
      private void TestEncodeDecodeArray(bool fromStream)
      {
         ...
         array[0].Capabilities = new Symbol[] { Symbol.Lookup("1") };
         array[1].Capabilities = new Symbol[] { Symbol.Lookup("1"), Symbol.Lookup("2") };
         array[2].Capabilities = new Symbol[] { Symbol.Lookup("1"), Symbol.Lookup("2"), Symbol.Lookup("3") };
         ...
         Assert.AreEqual(typeof(Coordinator), result.GetType().GetElementType());
         ...
         for (int i = 0; i < resultArray.Length; ++i)
         {
            ...
            Assert.IsTrue(resultArray[i] is Coordinator);
            Assert.AreEqual(array[i].Capabilities, resultArray[i].Capabilities);
         }
      }
