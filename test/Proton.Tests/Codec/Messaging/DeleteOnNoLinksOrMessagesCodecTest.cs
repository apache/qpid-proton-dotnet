   ...
   public class DeleteOnNoLinksOrMessagesTypeCodecTest : CodecTestSupport
   {
      ...
      private void DoTestDecodeFailsWhenArrayOfValuesSizeIsToLarge(EncodingCodes arrayType, bool fromStream)
      {
         ...
         if (fromStream)
         {
            ...
            Assert.IsTrue(typeDecoder.ReadValue(stream, streamDecoderState) is DeleteOnNoLinksOrMessages[]);
         }
         else
         {
            ITypeDecoder typeDecoder = decoder.ReadNextTypeDecoder(buffer, decoderState);
            Assert.AreEqual(typeof(Array), typeDecoder.DecodesType);
            Assert.IsTrue(typeDecoder.ReadValue(buffer, decoderState) is DeleteOnNoLinksOrMessages[]);
         }
